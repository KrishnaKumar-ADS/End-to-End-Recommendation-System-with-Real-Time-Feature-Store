...
export default function MovieCard({
  ...
  // Relative list score display: top item in this list is always 100.
  const rawScore = Number(movie.score) || 0;
  const safeTopScore = topScore > 0 ? topScore : 1;
  const relativeScore = (rawScore / safeTopScore) * 100;
  const scorePercent = Math.max(0, Math.min(100, Math.round(relativeScore)));
